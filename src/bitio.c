 ...
 ***************************************************************************/
int bitof(int n) { return (int)(ceil(log(n) / log(2))); }
 ...
 * 	Name        : bitIO_ferror - allows to check if a read mode opened
 *bitFILE has the error indicator set. Parameters  : bitF - bitFILE opened in
 *read mode Returned    : 0 if the flag is not set, non-zero value if it is set
 ***************************************************************************/
int bitIO_ferror(struct bitFILE *bitF) { return (ferror(bitF->file)); }
 ...
 * 				  mode - specify the mode: read(BIT_IO_R) or
 *write(BIT_IO_W) Returned    : bitFILE just opened in the specified mode
 ***************************************************************************/
 ...
 * 	Parameters  : bitF - bitFILE opened in write mode
 * 				  info - buffer containing information to be
 *written nbytes - number of bytes to be written from the buffer nbit - number
 *of bits required to represent the information Returned    : # bits written
 *successfully, -1 if error on inputs
 ***************************************************************************/
int bitIO_write(struct bitFILE *bitF, void *info, int nbit) {
  ...
  /* errors handler */
  if (bitF == NULL || bitF->file == NULL || bitF->mode != BIT_IO_W ||
      info == NULL || nbit < 0)
    return -1;
...
/***************************************************************************
 *								BIT I/O READ
 *FUNCTION Name        : bitIO_read - reads the first 'nbit' from the bitFILE
 *and puts them in the buffer. If an error occurs, or the end of the file is
 *      		  reached, the return value is a short item count (or
 *zero). bitIO_read() does not distinguish between end-of-file and error, and
 *callers must use 'bitIO_feof' and 'bitIO_ferror' to determine which occurred.
 * 	Parameters  : bitF - bitFILE opened in read mode
...
int bitIO_read(struct bitFILE *bitF, void *info, int info_s, int nbit) {
  ...
  /* errors handler */
  if (bitF == NULL || bitF->file == NULL || bitF->mode != BIT_IO_R ||
      info == NULL || info_s <= 0 || nbit < 0)
    return -1;
