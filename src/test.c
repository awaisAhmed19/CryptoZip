...
// PKCS#7 padding
int pad(unsigned char *data, int length) {
...
int main(int argc, char *argv[]) {
    ...
    long paddedSize = ((fileSize / BLOCK_SIZE) + 1) * BLOCK_SIZE;
    unsigned char *buffer = (unsigned char *)calloc(paddedSize, sizeof(unsigned char));
    fread(buffer, 1, fileSize, inFile);
    fclose(inFile);

    int paddedLength = pad(buffer, fileSize);

    unsigned char key[16] = {'k', 'k', 'k', 'k', 'e', 'e', 'e', 'e',
                             'y', 'y', 'y', 'y', '.', '.', '.', '.'};

    unsigned char *encrypted = (unsigned char *)malloc(paddedLength);
    unsigned char *decrypted = (unsigned char *)malloc(paddedLength);

    for (int i = 0; i < paddedLength; i += BLOCK_SIZE) {
        aes_encrypt(buffer + i, encrypted + i, key, SIZE_16);
    }

    FILE *outFile = fopen("encrypted.bin", "wb");
    fwrite(encrypted, 1, paddedLength, outFile);
    fclose(outFile);
    printf("Encryption complete. Saved as encrypted.bin\n");

    // Now decrypt for verification
    for (int i = 0; i < paddedLength; i += BLOCK_SIZE) {
        aes_decrypt(encrypted + i, decrypted + i, key, SIZE_16);
    }

    int unpaddedLength = unpad(decrypted, paddedLength);
    FILE *decFile = fopen("decrypted.txt", "wb");
    fwrite(decrypted, 1, unpaddedLength, decFile);
    fclose(decFile);
    printf("Decryption complete. Saved as decrypted.txt\n");
...
void core(unsigned char *word, int iteration) {
    ...
    // XOR the output of the rcon operation with i to the first part (leftmost)
    // only
...
void expandKey(unsigned char *expandedKey, unsigned char *key, enum keySize size,
               size_t expandedKeySize) {
    ...
    while (currentSize < expandedKeySize) {
        ...
        if (size == SIZE_32 && ((currentSize % size) == 16)) {
            for (i = 0; i < 4; i++) t[i] = getSBoxValue(t[i]);
        }

        /* We XOR t with the four-byte block 16,24,32 bytes before the new expanded
         * key. This becomes the next four bytes in the expanded key.
         */
        for (i = 0; i < 4; i++) {
...
void subBytes(unsigned char *state) {
     ...
     */
    for (i = 0; i < 16; i++) state[i] = getSBoxValue(state[i]);
}

void shiftRows(unsigned char *state) {
    int i;
    // iterate over the 4 rows and call shiftRow() with that row
    for (i = 0; i < 4; i++) shiftRow(state + i * 4, i);
}
...
void mixColumn(unsigned char *column) {
    ...
    column[0] = galois_multiplication(cpy[0], 2) ^ galois_multiplication(cpy[3], 1) ^
                galois_multiplication(cpy[2], 1) ^ galois_multiplication(cpy[1], 3);

    column[1] = galois_multiplication(cpy[1], 2) ^ galois_multiplication(cpy[0], 1) ^
                galois_multiplication(cpy[3], 1) ^ galois_multiplication(cpy[2], 3);

    column[2] = galois_multiplication(cpy[2], 2) ^ galois_multiplication(cpy[1], 1) ^
                galois_multiplication(cpy[0], 1) ^ galois_multiplication(cpy[3], 3);

    column[3] = galois_multiplication(cpy[3], 2) ^ galois_multiplication(cpy[2], 1) ^
...
void invSubBytes(unsigned char *state) {
     ...
     * using the state value as index for the SBox
     */
    for (i = 0; i < 16; i++) state[i] = getSBoxInvert(state[i]);
}

void invShiftRows(unsigned char *state) {
    int i;
    // iterate over the 4 rows and call invShiftRow() with that row
    for (i = 0; i < 4; i++) invShiftRow(state + i * 4, i);
}
...
void invMixColumn(unsigned char *column) {
    ...
    column[0] = galois_multiplication(cpy[0], 14) ^ galois_multiplication(cpy[3], 9) ^
                galois_multiplication(cpy[2], 13) ^ galois_multiplication(cpy[1], 11);
    column[1] = galois_multiplication(cpy[1], 14) ^ galois_multiplication(cpy[0], 9) ^
                galois_multiplication(cpy[3], 13) ^ galois_multiplication(cpy[2], 11);
    column[2] = galois_multiplication(cpy[2], 14) ^ galois_multiplication(cpy[1], 9) ^
                galois_multiplication(cpy[0], 13) ^ galois_multiplication(cpy[3], 11);
    column[3] = galois_multiplication(cpy[3], 14) ^ galois_multiplication(cpy[2], 9) ^
