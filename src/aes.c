...
void expandKey(unsigned char *expandedKey, unsigned char *key, enum keySize size,
    ...
    ASSERT(expandedKey != NULL);
    ASSERT(key != NULL);
    ASSERT(size == SIZE_16 || size == SIZE_24 || size == SIZE_32);
    ASSERT(expandedKeySize >= (size == SIZE_16 ? 176 : size == SIZE_24 ? 208 : 240));
    int currentSize = 0;
    int rconIteration = 1;
    int i;
...
void subBytes(unsigned char *state) {
    ASSERT(state != NULL); /* substitute all the values from the state with the value in the SBox
...
void shiftRows(unsigned char *state) {
    ASSERT(state != NULL);  // iterate over the 4 rows and call shiftRow() with that row
...
char aes_encrypt(unsigned char *input, unsigned char *output, unsigned char *key,
    ...
    switch (size) {
        ...
        case SIZE_32:
            ...
            break;
        default:
            ASSERT(0 && "Unknown AES key size");  // this should never happen if enum is correct
    ...
    ASSERT(expandedKey != NULL && "Memory allocation failed for expandedKey");

    if (expandedKey == NULL) {
        return ERROR_MEMORY_ALLOCATION_FAILED;
    } else {
        // map input block to state array with transpose order check
        for (i = 0; i < 4; i++) {
            for (j = 0; j < 4; j++) {
                ASSERT((i * 4 + j) < 16);
                ASSERT((i + j * 4) < 16);
...
void invSubBytes(unsigned char *state) {
    ...
    int i;
    for (i = 0; i < 16; i++) {
        // State elements must be valid bytes; you could also check range if paranoid
        // ASSERT(state[i] <= 0xFF);
...
void invShiftRow(unsigned char *state, unsigned char nbr) {
    ASSERT(state != NULL);
    // nbr should be between 0 and 3 (valid AES row index)
    ASSERT(nbr <= 3);
...
void aes_invMain(unsigned char *state, unsigned char *expandedKey, int nbrRounds) {
    ASSERT(state != NULL);
    ASSERT(expandedKey != NULL);
    ASSERT(nbrRounds > 0 && nbrRounds < 20);  // AES max rounds are 14, so 20 is a safe ceiling
